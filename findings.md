...
- 用户实测异步 writer 后删除仍不流畅，需要从终端输入显示语义层面处理，而不是只优化串口写入。
...
- Rust writer channel 后，按键发送已经基本不在主 UI 同步链路里；如果删除仍不流畅，主要原因更可能是显示等待串口对端回显。
- 串口 shell 常见行为是设备回显输入和退格擦除序列；如果前端完全等回显，Backspace 的视觉反馈会受设备处理、驱动缓冲、读线程事件派发影响。
- 类 PowerShell/Windows Terminal 的手感应由本地输入预览提供：本地先写入字符或擦除字符，然后短时间内吞掉设备回传的同样 echo，避免双显。
- 中文字符在终端通常占 2 个单元格，Backspace 本地擦除需要写两组 `\b \b`，否则容易出现光标覆盖或右侧残影。
...
| 输入预览默认开启 | 解决“按下删除后等设备回显才变化”的手感瓶颈 |
| 输入预览保留开关 | 遇到关闭回显、密码输入或全屏程序时，可恢复纯串口回显模式 |
| Backspace 立即发送 | 删除属于交互控制输入，不应被普通文本批量延迟 |
...
| 异步写入后仍删除不流畅 | 增加本地输入预览、回显吞吐和宽字符擦除 |
