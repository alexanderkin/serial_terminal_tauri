...
ADB 启动慢导致误判超时的问题已优化；等待用户实机确认首次启动 ADB server、刷新设备和远程连接不再过早超时。
...
- [x] 盘点 ADB 超时常量和 `run_adb_command` 调用点
- [x] 增加首次 `adb start-server` 的独立长超时窗口
- [x] 将普通 ADB 命令和远程连接命令超时从 3/4 秒放宽
- [x] 构建与检查验证
- **状态：** complete
...
| ADB 首次启动和具体命令分开设置超时 | 首次 `adb start-server` 可能很慢，应给更长窗口；真正的 `adb connect/devices/disconnect` 仍保留有限超时，避免设备不存在时长时间卡住 |
